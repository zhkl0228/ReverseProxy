...
int
buffer_has_space(Buffer *buf, size_t desired_length)
{
    size_t bytes_remaining = buf->total_size - buf->bytes_used;

    debug("Requesting %zu bytes, %zu available", desired_length, bytes_remaining);
...
buffer_grow(Buffer *buf, size_t minimum_size)
{
    ...
    debug("Growing buffer from %zu to %zu bytes", buf->total_size, new_size);
...
void
buffer_cat(Buffer *buf, char *append, size_t length)
{
    int i               = 0;
    int bytes_copied    = 0;
